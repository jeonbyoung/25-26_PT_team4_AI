import torch
import torch.nn as nn
import numpy as np



class Customed_NeRF(nn.Module):
    def __init__(self, pos_encd_dim = 10, dir_dim = 4, num_of_hidden_nodes = 128):
        super().__init__()

        self.box1 = nn.Sequential(nn.Linear(pos_encd_dim*6, num_of_hidden_nodes), nn.ReLU(),
                                  nn.Linear(num_of_hidden_nodes, num_of_hidden_nodes), nn.ReLU(),
                                  nn.Linear(num_of_hidden_nodes, num_of_hidden_nodes), nn.ReLU(),
                                  nn.Linear(num_of_hidden_nodes, num_of_hidden_nodes), nn.ReLU()
                                  )
        
        self.box2 = nn.Sequential(nn.Linear(pos_encd_dim*6 + num_of_hidden_nodes, num_of_hidden_nodes), nn.ReLU(),
                                  nn.Linear(num_of_hidden_nodes, num_of_hidden_nodes), nn.ReLU(),
                                  nn.Linear(num_of_hidden_nodes, num_of_hidden_nodes), nn.ReLU(),
                                  nn.Linear(num_of_hidden_nodes, num_of_hidden_nodes), nn.ReLU()
                                  )
        
        self.box3 = nn.Sequential(nn.Linear(num_of_hidden_nodes+dir_dim * 6, num_of_hidden_nodes // 2), nn.ReLU())
        self.density = nn.Sequential(nn.Linear(num_of_hidden_nodes, 1), nn.ReLU())

        self.box4 = nn.Sequential(nn.Linear(num_of_hidden_nodes // 2, 3), nn.Sigmoid())

        self.pos_encd_dim = pos_encd_dim
        self.dir_dim = dir_dim

    @staticmethod
    def positional_encoding(pixel_coordinate,L):
        ret = []
        for i in range(L):
            ret.append(torch.sin(2**i*np.pi*pixel_coordinate))
            ret.append(torch.cos(2**i*np.pi*pixel_coordinate))

        return torch.cat(ret, dim=1)
        
    def forward(self, pixel_coordinate, dir):
        pos_x = self.positional_encoding(pixel_coordinate, self.pos_encd_dim)
        pos_dir = self.positional_encoding(dir, self.dir_dim)

        session1 = self.box1(pos_x)
        session2 = self.box2(torch.cat((session1, pos_x), dim = 1))
        sigma = self.density(session2)
        session3 = self.box3(torch.cat((session2,pos_dir), dim=1))
        rgb = self.box4(session3)

        return rgb, sigma
    
    


